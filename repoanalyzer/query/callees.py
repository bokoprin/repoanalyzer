...
def find_callees(repo: str | Path, symbol: str, *, limit: int | None = None, offset: int = 0) -> list[CodeFact]:
    store = open_store(repo)
    facts = store.query_facts(active_fact_where("fact_type='call'"))
    matched = [fact for fact in facts if call_endpoint_matches(fact, symbol, "caller")]
