...
def find_references(repo: str | Path, symbol: str, *, limit: int | None = None, offset: int = 0) -> list[CodeFact]:
    store = open_store(repo)
    facts = store.query_facts(active_fact_where("fact_type='reference'"))
    matched = [fact for fact in facts if name_matches(fact.symbol, symbol, qualified=fact.qualified_name, subject=fact.object)]
