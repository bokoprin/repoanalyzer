# Qwen Code batch prompt: RepoAnalyzer TinyUSB golden E2E
...
Clineと比較するため、以下の10件を同じ条件で実行してください。
repoanalyzer MCPが利用できる場合は必ず使ってください。
各caseで、`collect_evidence` → 必要な `find_*` → `read_file_range` → JSONL回答、の順で進めてください。

Qwen Code上でMCP tool名や呼び出しUIがClineと異なる場合でも、実際に使ったtool名を `tool_trace` に記録してください。
MCPが見えない場合は、回答を作らず、各caseのJSONLに `known_limitations` として「repoanalyzer MCP unavailable」を記録してください。
